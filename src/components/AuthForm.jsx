...
export default function AuthForm({ onAuthed }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!validateEgyptianPhone(cleanPhone)) {
      setError('Please enter a valid Egyptian phone number (010, 011, 012, or 015).')
      return
    }
  ...
  return (
    <div className="auth-card" id="auth">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="field-group">
          <label className="field-label">Egyptian phone number</label>
          ...
          <span className="field-hint">11 digits starting with 010, 011, 012, or 015</span>
        </div>
        ...
        <button className="btn btn-primary auth-submit-btn" type="submit" disabled={loading}>
          {loading ? 'Please wait…' : mode === 'signup' ? 'Create Account & Book' : 'Log In to Book'}
        </button>
      </form>
    </div>
  )
